...
echo '<form method="post" 
            ...
            action="CG_001_001edf.php?ID='.$_GET['ID'].'&A='.$_GET['A'].'&ID_ob='.$_GET['ID_ob'].'&P='.$_GET['P'].'" />';
...
if ($_POST['Cargar'] == 'Cargar Factura') {
  if ($mensaje == '') {
    ...
    if ($valor_alim == 'Aliment Loc') {
      ...
      echo "Error: \$mensaje is not empty.<br>";
  }
}
/* fin parte 1 */
...
if ($_POST['Validar'] == 'Validar Archivo') {
  ...
  if ($mensaje == '') {
    ...
    //seleccionar id xml
    $sql_ed_xml = "SELECT id_archivo FROM CG_archivos where id_comprobacion = $SelID and tipo = 'xml'";
    $res_ed_xml = DB_query($sql_ed_xml,$db);
    $row_ed_xml = DB_fetch_array($res_ed_xml);
    $id_ed_xml = $row_ed_xml['id_archivo'];

    //seleccionar id pdf
    $sql_ed_pdf = "SELECT id_archivo FROM CG_archivos where id_comprobacion = $SelID and tipo = 'pdf'";
    $res_ed_pdf = DB_query($sql_ed_pdf,$db);
    $row_ed_pdf = DB_fetch_array($res_ed_pdf);
    $id_ed_pdf = $row_ed_pdf['id_archivo'];

    //actualizar xml
    $sql_up = "UPDATE CG_archivos 
                SET id_comprobacion = $SelID,
                    nombre = '$nombre_archivo_xml',
                    tipo = '$extension_xml'
                WHERE id_archivo = $id_ed_xml";
    $res_up = DB_query($sql_up,$db);
    
    //actualiza pdf
    $sql_up_pdf = "UPDATE CG_archivos
                   SET id_comprobacion = $SelID,
                       nombre = '$nombre_archivo_pdf',
                    tipo = '$extension_pdf'
                WHERE id_archivo = $id_ed_pdf";
...
if ($_POST['Validar'] == 'Validar Archivo') {
  ...
  echo '          <input type=Submit class="btn btn-primary" name="Cancelar" value="Cancelar">';
  echo '        </div>
              </div>
            </div>      
          </div>
